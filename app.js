...
const STORAGE_KEY = "school_schedule_v2";
...
// state = {
//   teachers: [...],
//   classes: [ { id, name, classType, schedule: { "rowId|dayKey": {teacherId,subject} } } ],
//   activeClassId: "..."
// }
let state = {
  teachers: [],
...
function load() {
  try {
    ...
    // Migração do formato v1 (turma única) para v2 (múltiplas turmas)
    const legacy = localStorage.getItem(LEGACY_STORAGE_KEY);
    if (legacy) {
      const old = JSON.parse(legacy);
      const cid = uid();
      state.teachers = old.teachers || [];
      state.classes = [{
        id: cid,
...
async function removeTeacher(id) {
  ...
  if (!t) return;
  const ok = await confirmModal(`Remover ${t.name}? Todos os horários dele(a) em todas as turmas serão limpos.`);
  if (!ok) return;
  state.teachers = state.teachers.filter(x => x.id !== id);
  // Limpa slots deste professor em TODAS as turmas
...
// Se o professor `teacherId` já está lotado em outra turma no mesmo
// (turno, aula, dia), retorna a turma conflitante. Caso contrário, null.
function findCrossClassConflict(teacherId, targetClassId, targetClassType, targetRowId, targetDayKey) {
...
function handleDrop(rowId, dayKey, payload) {
  ...
  // 2. Choque entre turmas: o mesmo professor não pode estar em duas
  //    turmas no mesmo dia+horário (mesmo turno e mesma aula).
  const conflict = findCrossClassConflict(
    payload.teacherId, cls.id, cls.classType, rowId, dayKey
  );
  if (conflict) {
    const teacher = state.teachers.find(x => x.id === payload.teacherId);
...
function autoFill() {
  ...
  // para evitar choques entre turmas no preenchimento automático.
  const otherClassBusy = new Set(); // "teacherId|dayKey|Turno|Aula"
  for (const other of state.classes) {
    ...
    for (const k of Object.keys(other.schedule)) {
      ...
      otherClassBusy.add(`${e.teacherId}|${dayKey}|${label}`);
    }
  }
  ...
  for (const unit of units) {
    ...
    for (const slot of freeSlots) {
      ...
      // Bloqueia se o professor já está em outra turma no mesmo dia+horário
      const label = slotTimeLabel(cls.classType, slot.rowId);
      if (otherClassBusy.has(`${unit.teacherId}|${slot.dayKey}|${label}`)) continue;
    ...
    otherClassBusy.add(`${unit.teacherId}|${best.dayKey}|${label}`);
  }
...
function seedIfEmpty() {
  if (state.classes.length === 0 && state.teachers.length === 0) {
    const cid = uid();
    state.classes = [{
      id: cid, name: "4° A", classType: "regular", schedule: {},
    }];
    state.activeClassId = cid;
    state.teachers = [
