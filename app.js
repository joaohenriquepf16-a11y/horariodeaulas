...
function normalizeSubject(name) {
  ...
  const map = {
    ...
    "LIBRAS":    "LIB",
    "LIB":       "LIB",
    "ATIVIDADE SUPERVISIONADA": "AT SUP",
    "AT SUP":    "AT SUP",
    "AT. SUP":   "AT SUP",
    "ATSUP":     "AT SUP",
    "FOCO SPAECE":"F SPA",
...
function renderClasses() {
  ...
  const typeLabel = {
    ...
    integral_6a8: "6° ao 8° ano (Integral)",
    integral_9: "9° ano (Integral)",
  };
...
function autoFill() {
  ...
  // Considera SOMENTE as quantidades cadastradas por professor/disciplina
  // (subjectQuotas). Nada é acrescentado além do que foi cadastrado.
  const items = [];
  teachersInClass.forEach(t => {
    const teacherQuotas = t.subjectQuotas || {};
    (t.subjects || []).forEach(s => {
      const planned = parseInt(teacherQuotas[s] || 0, 10) || 0;
      const alreadyUsed = combinationUsageCount(t.id, s);
      const need = planned - alreadyUsed;
      for (let i = 0; i < need; i++) {
        items.push({ teacherId: t.id, subject: s, count: 1 });
      }
    });
  });

  if (items.length === 0) {
    toast("Quantidades cadastradas já atendidas — nada a preencher.", "error");
    return;
  }
...
// ------- Init -------
function bindUI() {
  // Auto-detecção: quando o usuário digita o nome da turma,
  // se o texto sugerir 6°, 7°, 8° ou 9° ano (Integral),
  // o tipo da grade é selecionado automaticamente.
  function detectClassTypeFromName(rawName) {
    const n = (rawName || "")
      .toUpperCase()
      .replace(/º/g, "°")
      .replace(/\s+/g, " ")
      .trim();
    if (!n) return null;
    // Detecta menção a "integral" (obrigatório para as novas grades)
    const isIntegral = /INTEGRAL/.test(n);
    // Extrai o primeiro dígito de ano (6, 7, 8 ou 9)
    // Aceita formatos como "6°", "6 ANO", "6°A", "6ºANO", "9°B", etc.
    const m = n.match(/\b([6-9])\s*°?\s*(?:ANO)?\b/);
    if (!m) return null;
    const ano = m[1];
    // Só aplica se o nome indicar integral OU se o tipo atual já for integral variante
    if (!isIntegral) return null;
    if (ano === "9") return "integral_9";
    // 6, 7 ou 8
    return "integral_6a8";
  }

  const newClassNameInput = $("#newClassName");
  const newClassTypeInput = $("#newClassType");
  if (newClassNameInput && newClassTypeInput) {
    const syncClassType = () => {
      const detected = detectClassTypeFromName(newClassNameInput.value);
      if (detected) newClassTypeInput.value = detected;
    };
    newClassNameInput.addEventListener("input", syncClassType);
    newClassNameInput.addEventListener("blur", syncClassType);
  }

  $("#btnAddClass").addEventListener("click", () => {
    const name = $("#newClassName").value.trim();
    // Reaplica detecção antes de adicionar, caso o usuário não tenha disparado o input event
    const detected = detectClassTypeFromName(name);
    if (detected) $("#newClassType").value = detected;
  ...
  // Ao digitar o nome de um professor já cadastrado nesta turma,
  // recupera a cor original dele no seletor de cor.
  const teacherNameInput = $("#teacherName");
  const teacherColorInput = $("#teacherColor");
  if (teacherNameInput && teacherColorInput) {
    const syncTeacherColor = () => {
      const cls = activeClass();
      if (!cls) return;
      const typed = (teacherNameInput.value || "").trim().toLowerCase();
      if (!typed) return;
      const existing = (cls.teachers || []).find(
        t => (t.name || "").toLowerCase() === typed
      );
      if (existing && existing.color) {
        teacherColorInput.value = existing.color;
      }
    };
    teacherNameInput.addEventListener("input", syncTeacherColor);
    teacherNameInput.addEventListener("blur", syncTeacherColor);
  }
